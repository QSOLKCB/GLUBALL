    ...
    pub fn new(
        ...
        if !(major_radius.raw() > minor_radius.raw() && minor_radius.raw() > 0) {
            return Err(RuntimeError(
                "require major_radius > minor_radius > 0".into(),
            ));
        }
        if !(tube_radius.raw() > 0 && tube_radius.raw() < minor_radius.raw()) {
            return Err(RuntimeError(
                "require 0 < tube_radius < minor_radius".into(),
            ));
        }
        Ok(Self {
            major_radius,
...
impl Vec3 {
    ...
    fn normalize(self) -> Result<Self, RuntimeError> {
        ...
        if !n.is_finite() || n <= 0.0 {
            return Err(RuntimeError("cannot normalize vector".into()));
        }
        Ok(self.scale(1.0 / n))
    }
}
...
fn checked_angle(value: f64) -> Result<f64, RuntimeError> {
    if !value.is_finite() || value.abs() > MAX_ANGLE_ABS {
        return Err(RuntimeError(
            "angle outside bounded finite runtime domain".into(),
        ));
    }
    Ok(value)
}
fn angle_from_index(index: u32, count: u32) -> Result<f64, RuntimeError> {
    if count == 0 || index >= count {
        return Err(RuntimeError("mesh index outside domain".into()));
    }
...
pub fn surface_point_indices(
    ...
    Ok(centre
        .add(n.scale(a.cos() * rho))
        .add(b.scale(a.sin() * rho)))
}
...
impl SamplingPolicy {
    ...
    pub fn parse(s: &str) -> Result<Self, RuntimeError> {
        match s {
            ...
            _ => Err(RuntimeError(format!("unknown sampling policy: {s}"))),
        }
    }
}
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl SamplingConfig {
    pub fn new(
        ...
        if logical_count == 0 {
            return Err(RuntimeError("logical_count must be > 0".into()));
        }
        if rendered_count == 0 || rendered_count > MAX_RENDERED_COUNT {
            return Err(RuntimeError(
                "rendered_count outside runtime bound".into(),
            ));
        }
        if u64::from(rendered_count) > logical_count {
            return Err(RuntimeError(
                "rendered_count may not exceed logical_count".into(),
            ));
        }
        Ok(Self {
            logical_count,
    ...
    pub fn logical_index(self, i: u32) -> Result<u64, RuntimeError> {
        if i >= self.rendered_count {
            return Err(RuntimeError("rendered index outside domain".into()));
        }
        match self.policy {
            SamplingPolicy::UniformFloor => Ok(((u128::from(i)
                * u128::from(self.logical_count))
                / u128::from(self.rendered_count)) as u64),
...
pub fn partition_ranges(items: u64, requested: usize) -> Result<Vec<WorkRange>, RuntimeError> {
    if items == 0 {
        return Err(RuntimeError("item count must be > 0".into()));
    }
    if requested == 0 || requested > MAX_WORKERS {
        return Err(RuntimeError("worker count outside runtime bound".into()));
    }
    ...
    if cursor != items {
        return Err(RuntimeError("partition coverage invariant failed".into()));
    }
    Ok(out)
}

/// Validated simulation dimensions and execution bounds.
///
/// Fields are private so callers cannot bypass `SimulationConfig::new` and
/// construct an overflowing or out-of-contract execution domain.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl SimulationConfig {
    pub fn new(
        ...
        if !(12..=1_000_000).contains(&u) {
            return Err(RuntimeError("u_segments outside [12,1000000]".into()));
        }
        if !(6..=65_536).contains(&v) {
            return Err(RuntimeError("v_segments outside [6,65536]".into()));
        }
        if repeats == 0 {
            return Err(RuntimeError("repeats must be > 0".into()));
        }
        if workers == 0 || workers > MAX_WORKERS {
            return Err(RuntimeError("workers outside runtime bound".into()));
        }
        if device_slots == 0 || device_slots > MAX_DEVICE_SLOTS {
            return Err(RuntimeError("device slots outside runtime bound".into()));
        }
        u64::from(u)
            .checked_mul(u64::from(v))
            .and_then(|x| x.checked_mul(u64::from(repeats)))
            .ok_or_else(|| RuntimeError("total point count overflow".into()))?;
        Ok(Self {
            u_segments: u,
            v_segments: v,
            repeats,
    ...
    pub fn u_segments(self) -> u32 {
        self.u_segments
    }
    pub fn v_segments(self) -> u32 {
        self.v_segments
    }
    ...
    pub fn total_points(self) -> Result<u64, RuntimeError> {
        u64::from(self.u_segments)
            .checked_mul(u64::from(self.v_segments))
            .and_then(|x| x.checked_mul(u64::from(self.repeats)))
            .ok_or_else(|| RuntimeError("total point count overflow".into()))
    }
}
...
fn fnv(mut state: u64, word: u64) -> u64 {
...
/// Position-bind one output record before the commutative shard fold. This
/// keeps the aggregate diagnostic independent of worker/shard boundaries while
/// still distinguishing equal point values at different linear positions.
fn record_hash(linear: u64, point: Vec3) -> u64 {
    let mut hash = FNV_OFFSET;
    hash = fnv(hash, linear);
    hash = fnv(hash, point.x.to_bits());
    hash = fnv(hash, point.y.to_bits());
    fnv(hash, point.z.to_bits())
}
...
fn run_worker(
    ...
    let per_repeat = u64::from(sim.u_segments)
        .checked_mul(u64::from(sim.v_segments))
        .ok_or_else(|| RuntimeError("per-repeat point count overflow".into()))?;
    let mut hash = 0_u64;
...
pub fn simulate(
    ...
    let total_points = sim.total_points()?;
    let ranges = partition_ranges(total_points, sim.workers)?;
    let summaries = thread::scope(|scope| {
        let handles: Vec<_> = ranges
            .iter()
            .copied()
            .map(|r| scope.spawn(move || run_worker(r, sim, c)))
            .collect();
        let mut out = Vec::with_capacity(handles.len());
        for h in handles {
            out.push(
                h.join()
                    .map_err(|_| RuntimeError("worker thread panicked".into()))??,
            );
        }
    ...
    for worker in &summaries {
        max_err = max_err.max(worker.max_tube_radius_error);
        hash ^= worker.diagnostic_hash_fnv1a64;
    }
...
mod tests {
    ...
    #[test]
    fn fixed_parameters_are_exact() {
        let c = GeometryConfig::canonical();
        assert_eq!(c.major_radius.raw(), 2_100_000);
        assert_eq!(c.minor_radius.raw(), 850_000);
        assert_eq!(c.tube_radius.raw(), 340_000);
    }

    #[test]
    fn uniform_vectors() {
        let c = SamplingConfig::new(1 << 24, 96, SamplingPolicy::UniformFloor).unwrap();
        for (i, e) in [
            (0, 0),
            (1, 174_762),
            (2, 349_525),
            ...
            assert_eq!(c.logical_index(i).unwrap(), e);
        }
    }

    #[test]
    fn phi_vectors() {
        let c = SamplingConfig::new(1 << 24, 96, SamplingPolicy::PhiWeyl64).unwrap();
        for (i, e) in [
            (0, 0),
            (1, 10_368_889),
            (2, 3_960_563),
    ...
    #[test]
    fn large_vector() {
        let c = SamplingConfig::new(1_u64 << 32, 4096, SamplingPolicy::UniformFloor).unwrap();
        for (i, e) in [
            (0, 0),
            (1, 1_048_576),
            (2047, 2_146_435_072),
    ...
    fn partitions_cover() {
        ...
        assert_eq!(ranges.iter().map(|x| x.length).sum::<u64>(), 4097);
    ...
    #[test]
    fn diagnostic_hash_is_worker_sharding_independent() {
        let single = simulate(
            SimulationConfig::new(96, 18, 2, 1, 1).unwrap(),
            GeometryConfig::canonical(),
        )
        .unwrap();
        let sharded = simulate(
            SimulationConfig::new(96, 18, 2, 8, 2).unwrap(),
            GeometryConfig::canonical(),
        )
        .unwrap();
        assert_eq!(single.total_points, sharded.total_points);
        assert_eq!(single.diagnostic_hash_fnv1a64, sharded.diagnostic_hash_fnv1a64);
    }
}
